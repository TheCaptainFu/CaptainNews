...
const RSS_FEEDS = {
    breaking: { title: 'ΕΚΤΑΚΤΗ ΕΠΙΚΑΙΡΟΤΗΤΑ & ΕΛΛΑΔΑ', color: 'red' },
    politicsGR: { title: 'ΠΟΛΙΤΙΚΑ (ΕΛΛΑΔΑ)', color: 'purple' },
    worldPolitics: { title: 'ΠΟΛΙΤΙΚΑ (ΠΑΓΚΟΣΜΙΩΣ)', color: 'blue' },
    sports: { title: 'ΑΘΛΗΤΙΣΜΟΣ & SUPER LEAGUE', color: 'green' },
    panathinaikos: { title: 'ΠΑΝΑΘΗΝΑΪΚΟΣ', color: 'lime' },
    tech: { title: 'ΤΕΧΝΟΛΟΓΙΑ & SCIENCE', color: 'pink' }
};
...
async function getNews() {
    try {
        ...
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        console.log(`🚀 Fetching news from news.json at ${fetchTime.toLocaleTimeString('el-GR')}`);
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        console.log(`⏰ Last updated: ${new Date(newsData.lastUpdated).toLocaleString('el-GR')}`);
        ...
        if (!autoRefreshInterval && !nextRefreshTime) {
            ...
            console.log('✓ Auto-refresh timer already running');
            if (nextRefreshTime) {
                console.log(`⏰ Next refresh at: ${new Date(nextRefreshTime).toLocaleTimeString('el-GR')}`);
            }
                    ...
                    <div class="mb-8 max-h-[400px] bg-[#1a1a1a] rounded-xl border border-zinc-800 ${styles.borderHover} transition-all duration-300 hover:shadow-2xl ${styles.shadow} overflow-hidden">
                        <a href="${art.link}" target="_blank" class="block group h-full">
                            <div class="grid md:grid-cols-2 gap-0 h-full min-h-0">
                                ...
                                `}
                                <div class="p-6 md:p-8 flex flex-col min-h-[300px] order-2 max-w-xl">
                                    <h3 class="text-xl md:text-2xl lg:text-3xl font-bold text-zinc-100 ${styles.titleHover} transition-colors mb-3 md:mb-4 leading-tight">
                                        ${art.title}
                                    </h3>
                                    ${longDescription ? `<p class="text-zinc-400 text-sm md:text-base leading-relaxed mb-4 md:mb-6 line-clamp-4">${longDescription}</p>` : ''}
                                    <div class="mt-auto">
                                        ${art.source ? `<p class="text-white text-xs md:text-sm mb-3 md:mb-4">Πηγή: ${art.source}</p>` : ''}
                                        <div class="flex flex-wrap items-center gap-3 pt-4 border-t border-zinc-800">
                                            ...
                                            <span class="${styles.readMore} text-xs md:text-sm font-semibold opacity-100 flex items-center gap-2">
...
function startCountdownTimer() {
    ...
    const updateCountdown = () => {
        if (!nextRefreshTime) {
            countdownElement.textContent = 'Αναμονή...';
            return;
        }
        ...
        if (timeRemaining <= 0) {
            countdownElement.textContent = 'Ανανέωση τώρα...';
        } else {
            countdownElement.textContent = `Επόμενη ανανέωση σε ${timeString}`;
        }
    };
